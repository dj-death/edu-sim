...
namespace EduSim.CoreFramework.Common
{
    ...
    public class CatalogSettings
    {
        ...
        private static List<CatalogSettings> uploadedCatalogConfigurations = null;
        ...
        static CatalogSettings()
        {
            ...
            uploadedCatalogConfigurations = CatalogSettings.GetUploadedInstance();
        }
        ...
        internal XmlElement CATConfigSection
        {
            get
            {
        ...
        public static List<CatalogSettings> UploadedCatalogSections
        {
            get
            {
                return uploadedCatalogConfigurations;
            }
        }
        ...
        internal static List<CatalogSettings> GetUploadedInstance()
        {
            List<CatalogSettings> uploadedCatalogConfigurations = new List<CatalogSettings>();

            DirectoryInfo di = new DirectoryInfo(HttpContext.Current.Server.MapPath("~/"));
            foreach (FileInfo fi in di.GetFiles("Module-*.xml"))
            {
                using(FileStream stream = new FileStream(HttpContext.Current.Server.MapPath("~/" + fi.Name), FileMode.Open, FileAccess.Read))
                {
                    XmlDocument xmlDoc = new XmlDocument();

                    xmlDoc.Load(stream);
                    uploadedCatalogConfigurations.Add( new CatalogConfigHandler().Create(null, null, xmlDoc.FirstChild) as CatalogSettings);
                }
            }
